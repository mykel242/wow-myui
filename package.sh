...
# Function to extract all lua files from TOC
extract_lua_files_from_toc() {
    local toc_file="MyUI2.toc"
    if [ -f "$toc_file" ]; then
        # Extract all lua files from TOC that aren't commented
        grep -v "^#" "$toc_file" | grep "\.lua$" | sed 's/^[[:space:]]*//' | sed 's/[[:space:]]*$//'
    fi
}
